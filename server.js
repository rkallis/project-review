...
function createComment(url, request) {
  ...
  if (requestComment && database.users[requestComment.username] && requestComment.body && database.articles[requestComment.articleId]) { //Value checks
      const comment = {
    ...
    database.comments[comment.id] = comment; // Add comments to database
    database.users[comment.username].commentIds.push(comment.id); // Pushes comment id to user Object
    database.articles[comment.articleId].commentIds.push(comment.id); // Pushes comment id to article Object
...
function updateComment(url, request) {
  ...
  if (!id || !requestComment) {
    ...
    const comment = {  // Make a new comment object, where only change is the body
      id: savedComment.id,
      body: requestComment.body,
      username: savedComment.username,
      articleId: savedComment.articleId,
      upvotedBy: savedComment.upvotedBy,
      downvotedBy: savedComment.downvotedBy
    }
    database.comments[comment.id] = comment; // Overwrite existing comment with new Object
    response.body = {comment: comment};
    response.status = 200;
  }
  return response;
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    database.comments[id] = null; // Removes comment from database

    const userComments = database.users[savedComment.username].commentIds;
    userComments.splice(userComments.indexOf(id), 1); // Remove comment id from user.commentsIds.

    const articleComments = database.articles[savedComment.articleId].commentIds;
    articleComments.splice(articleComments.indexOf(id), 1); // Remove comment id from article.commentsIds.
...
function upvoteComment(url, request) {
  ...
  if (savedComment && database.users[username]) {
    ...
    if (!savedComment.upvotedBy.includes(username)) { // Check if username has already upvoted

      if (savedComment.downvotedBy.includes(username)) {
        savedComment.downvotedBy.splice(savedComment.downvotedBy.indexOf(username), 1); // Check and remove comment from downvotedBy
      }
      savedComment.upvotedBy.push(username); // Pushed username to savedComment object that will be returned;
      database.comments[id] = savedComment; // Updates the comments object with the modified version.

      response.body = {comment: savedComment};
      response.status = 200;

    } else {
      response.status = 400; // Status when user already upvoted.
    }

  } else {
    response.status = 400; // Status when invalid entries.
  }
  return response;
};
...
function downvoteComment(url, request) {
  ...
  if (savedComment && database.users[username]) {
    ...
    if (!savedComment.downvotedBy.includes(username)) { // Check if username has already downvoted

      if (savedComment.upvotedBy.includes(username)) {
        savedComment.upvotedBy.splice(savedComment.upvotedBy.indexOf(username), 1); // Check and remove comment from downvotedBy
      }
      savedComment.downvotedBy.push(username); // Pushed username to savedComment object that will be returned;
      database.comments[id] = savedComment; // Updates the comments object with the modified version.
      ...
      response.status = 400; // Status when user already downvoted
    }
...
// Bonus. Testing with YAML. Will continue later.
...
// ************************************************
// End of own code, and start of project provided code
// *************************************************
